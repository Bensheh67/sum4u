...
def select_keyframes(transcript: str, video_duration: float = None,
    ...
    try:
        ...
        print("[DEBUG] select_keyframes JSON解析失败，尝试正则提取...")
        # 尝试从代码块中提取 JSON
        match = re.search(r'```(?:json)?\s*(\{[\s\S]*?\})\s*```', content, re.DOTALL)
        if match:
            try:
                result = json.loads(match.group(1))
                print("[DEBUG] select_keyframes 从代码块中提取JSON成功")
            ...
            try:
                ...
                if json_match:
                    ...
                    print("[DEBUG] select_keyframes 正则提取JSON成功")
                else:
                    print("[DEBUG] select_keyframes 未找到JSON对象")
                    result = None
        ...
        if result is None:
            print("[ERROR] select_keyframes 无法解析AI返回内容，返回空列表")
            return []
